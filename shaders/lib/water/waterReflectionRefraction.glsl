...
vec2 SSRT(vec3 viewPos, vec3 reflectViewDir, vec3 normalTex, float roughness, out vec3 outMissPos){
    ...
    int dynamicSamples = int(max(4.0, float(REFLECTION_SAMPLES) * roughnessFactor));
...
vec3 reflection(sampler2D tex, vec3 viewPos, vec3 reflectWorldDir, vec3 reflectViewDir, 
                float lightmap, vec3 normalTex, float roughness, float colorScale, inout bool ssrTargetSampled){
